...
- **两种玩法**：`1` 单屏法官（一人主持）/ `2` 传递游玩（一台设备多人，清屏 + 私密交接，每人只看自己信息，无需网络）。
...
ctest --test-dir build --output-on-failure # 跑测试（88 个用例）
```
...
- `io/` — `DecisionProvider` 接口 + `Scripted`（测试）/ `Console`（真人）/ `PassAndPlay`（单设备多人）实现。
- `app/` — `main.cpp` 命令行入口。
...
法官单机工具（已完成）→ 随机发牌（已完成）→ 单设备传递游玩（已完成）→ 发言记录与复盘（已完成）→ 拍刀阶段 C 自动最优搜索 → 真·联机（多设备/网络，按玩家定向隐藏信息）→ 接入 AI agent 玩家。
