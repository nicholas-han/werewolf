...
> **实现现状（v1）**：`deal`/`speech`/`result_private`/`decision` 已带结构化 `data`；`phase`/`death`/`vote` 目前以 `narration` 文本发出（未结构化），`status` 为 moderator 文本。orchestrator 从公开叙述文本提取所需信息（如存活/出局）。结构化 `phase`/`death`/`vote`（上表 `data`）列为后续增强。

> **day/phase 标签**：每个事件都带 `day`/`phase` 上下文，orchestrator 与 recorder 据此把日志/script 分段。引擎在**进入每个相位时**先同步这对标签（`DecisionProvider::onPhaseEnter`，于 `runNight`/`runDay` 开头调用）、**再**发任何横幅/死讯——否则相位切换处经 `notify()`（签名不带 GameState）发出的首批叙述会沿用上一次 ask 的旧相位标签（如「第 2 夜」横幅、黎明死讯被错归到前一天/相位）。
...
- **狼刀**（秘密投票，BRD §2）：对每个存活睁眼狼发 `ask(NightKill, allowSkip)`（弃票=倾向空刀），每票记 `event(moderator, decision, {kind:"NightKillVote", target?})`；**严格最高票**被刀，平票/无人投票＝空刀。决议后向每个睁眼狼发 `event(private, narration, text="【狼队】今晚刀：Px / 空刀")`，并发 `event(moderator, decision, {kind:"NightKill", target?})` 入 script。机械狼独狼（狼队仅剩它）时单独定刀（单选）。
- **狼队私聊**（每晚刀前最多 2 轮，无机制影响）：对每个存活睁眼狼依次发 `ask(speak, kind=WolfChat)`；每条回复经 §6.4 转发给其他睁眼狼（private）+ 入 script。机械狼不参与。
...
- `reply.choice` 不在候选 / 类型错 / 该行 JSON 解析失败 / EOF → 引擎回退到**合法默认**；回退后的结果体现在该决策的 `decision`（moderator）事件里。规则：
  ...
  - speak：默认 `""`（视为过）。
- **软超时** `--ask-timeout`（默认 **600s**）：超时同样回退到合法默认并记 moderator 事件。仅作防挂死——正常路径由 orchestrator 端的 AgentBrain 兜底保证及时回复（见 `ai_agents_design.md` §7.4）。本地推理模型慢，故默认放宽。
