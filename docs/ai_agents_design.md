...
引擎是权威方，必须对坏输入鲁棒：
- `reply.choice` 不在候选集 / 类型错误 / JSON 解析失败 → 引擎回退到**合法默认**（`allowSkip` 则跳过；否则投票取第一个非自己、其余取首候选），并发一条 `vis=moderator` 的 `decision` 事件记录这次回退（与 protocol_v1.md §8 一致）。
- orchestrator 侧的超时/模型故障由 orchestrator 兜底（AgentBrain 自带回退，§7.4），最终总会给引擎一个**合法 reply**，使引擎不必等待无限期。
...
- `game.cpp::runNight`：在「**狼人请睁眼**」之后、做刀决策**之前**，插入一轮私聊（即夜间行动循环中、`狼人` 这组 openEyes cue 之后、`actAtNight` 之前触发；不能放在循环之前，否则会出现「先私聊后睁眼」的错误叙述）：
  ...
  - **每晚刀前都进行**；**默认 2 轮**（`--wolf-chat-rounds`，引擎 `Game::setWolfChatRounds`）。**自然收尾**：第 2 轮起提示「没有补充就回『过』」，某狼回空/「过」即视为弃言不广播；**整轮全员弃言则提前结束**（`isWolfChatPass` 判定，game.cpp 的 `runWolfChat` 轮循环 + anySpoke 早停）。
...
### 5.7 逐发言中断：自爆 / 退水（默认关，`--interrupts`）

「随时自爆/退水」在 AI 局里＝**每位发言之间**（AI 一段发言是原子的，没有「说到一半」）。`Game::setSpeechInterrupts(true)`（引擎 `--interrupts`，orchestrator `--interrupts` 透传）：
- **白天发言**：`collectDaySpeeches` 每位发言后轮询狼人自爆；有人自爆 → 立即结束白天（返回该狼，`runDay` 结算 BlownUp）。
- **警长候选人发言**：`runSheriffElection` 每位候选发言后，先查自爆（→ `resolveElectionSelfDestruct`，§7.4 中断）、再问各剩余候选退水（退到 1 即当选 §7.3）。
- **默认关**：用「发言后一个统一窗口」（旧行为，快）。旧路径逐字节不变，既有测试不受影响。
- **代价**：开启后每位发言后都要逐狼/逐候选询问（多为「否」），在本地推理模型上**很慢**（每天 O(#狼×#发言) 次 ~30s 询问）——故默认关，想要极致忠实时再开。§11 不泄露不变（自爆询问仍 `vis=moderator`，退水 `logMod=false`）。
...
parse_and_validate(out, ask):
  ...
  if qtype==choose:
     ...
     else: 合法默认（Ollama `format` 已强制 content 为合法 JSON、choice 取自候选 enum，失败率极低；当前不重试）
...
{ "game_id":"...", "seat":3, "role":"Witch", "day":2, "phase":"Night",
  ...
  "attempts":[ {"raw":"…非法输出…","error":"choice 不在候选"} ],  // 被回退的非法输出（保留字段；当前不重试，故恒为空）
  "latency_ms":48213,
...
| 2 | 狼私聊轮数 | **每晚刀前最多 2 轮**（默认，自然收尾）；自由往复留后续（§5.4）。 |
